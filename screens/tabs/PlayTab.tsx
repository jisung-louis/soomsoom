...
const PlayTab = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true);
        const shortActivityResponse = await Promise.resolve(getActivitiesByType('BREATHING')); //호흡 타입 액티비티 가져오기
        const allActivitiesResponse = await Promise.resolve(getActivities()); // 모든 액티비티를 가져온 후 랜덤으로 5개 선택
      ...
      ListHeaderComponent={
        <>
          ...
          <View style={styles.contentContainer}>
            <PlayMainCard onPress={onMainCardPress} />
